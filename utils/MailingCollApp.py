...
class MailingCollAPP(QtWidgets.QMainWindow, mailing.Ui_mailing):

    def __init__(self, Session, liste_dest, parent=None):
        super(MailingCollAPP, self).__init__(parent)
        try:
    ...
    def init_app(self):
        ...
        for dest in self.liste:
            a = s.query(assogestion_collectivite).filter_by(code_collectivite=dest.code_collectivite).first()
            if a.email_collectivite != "":
                self.liste_destinataire.addItem(a.email_collectivite)
        for index in range(self.liste_destinataire.count()):
            self.liste_dest.append(self.liste_destinataire.item(index).text())
        if self.liste_destinataire.count() == 1:
            self.label_dest.setText(f"{self.liste_destinataire.count()} destinataire")
        else:
            self.label_dest.setText(f"{self.liste_destinataire.count()} destinataires")
        s.close()
    ...
    def send_mail(self):
        ...
        if self.mail_message_edit.toPlainText() != "" and self.sujet_edit.text() != "":
